...
v0.11.0 tagged. 22 tools total (17 TMDB + 4 awards + 1 representation), 24 ceremonies, 101 award categories. M12 (box office & financial data) complete — `get_financials` tool aggregating TMDB + OMDb with optional OMDb API key. M15 (talent representation) complete — `get_person_representation` tool querying Wikidata P1875 for agency affiliations. M16-M20 planned — spans new tools, data expansion, research, workflow design, impact campaign data, plugin architecture, and a standalone philanthropic intelligence MCP.
